...
// @LINE:56
private[this] lazy val controllers_UserController_login38_route = Route("POST", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("userService/login"))))
private[this] lazy val controllers_UserController_login38_invoker = createInvoker(
...
// @LINE:57
private[this] lazy val controllers_UserController_loginTest39_route = Route("GET", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("userService/loginTest/json"))))
private[this] lazy val controllers_UserController_loginTest39_invoker = createInvoker(
play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.UserController]).loginTest(fakeValue[String]),
HandlerDef(this.getClass.getClassLoader, "", "controllers.UserController", "loginTest", Seq(classOf[String]),"GET", """""", Routes.prefix + """userService/loginTest/json"""))
...
// @LINE:56
case controllers_UserController_login38_route(params) => {
   call { 
        controllers_UserController_login38_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.UserController]).login)
   }
}
        

// @LINE:57
case controllers_UserController_loginTest39_route(params) => {
   call(Param[String]("format", Right("json"))) { (format) =>
        controllers_UserController_loginTest39_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.UserController]).loginTest(format))
   }
}
