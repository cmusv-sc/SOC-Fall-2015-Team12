...
// @LINE:56
private[this] lazy val controllers_UserController_loginWithUserNameAndPassword38_route = Route("GET", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("userService/login/userName/"),DynamicPart("userName", """[^/]+""",true),StaticPart("/password/"),DynamicPart("password", """[^/]+""",true),StaticPart("/json"))))
private[this] lazy val controllers_UserController_loginWithUserNameAndPassword38_invoker = createInvoker(
play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.UserController]).loginWithUserNameAndPassword(fakeValue[String], fakeValue[String]),
HandlerDef(this.getClass.getClassLoader, "", "controllers.UserController", "loginWithUserNameAndPassword", Seq(classOf[String], classOf[String]),"GET", """""", Routes.prefix + """userService/login/userName/$userName<[^/]+>/password/$password<[^/]+>/json"""))
        

// @LINE:57
private[this] lazy val controllers_UserController_login39_route = Route("POST", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("userService/login"))))
...
HandlerDef(this.getClass.getClassLoader, "", "controllers.UserController", "login", Nil,"POST", """""", Routes.prefix + """userService/login"""))
...
// @LINE:56
case controllers_UserController_loginWithUserNameAndPassword38_route(params) => {
   call(params.fromPath[String]("userName", None), params.fromPath[String]("password", None)) { (userName, password) =>
        controllers_UserController_loginWithUserNameAndPassword38_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.UserController]).loginWithUserNameAndPassword(userName, password))
   }
}
